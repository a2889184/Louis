...
int main(){
	fstream fin1, fin2, fin3, fin4, fin5, fin6, fin7, fin8, fin9, output;
	
	fin1.open("split_50/U_1.txt", ios::in);
	fin2.open("split_50/U_2.txt", ios::in);
	fin3.open("split_50/D_1.txt", ios::in);
	fin4.open("split_50/D_2.txt", ios::in);
	fin5.open("split_50/V_1.txt", ios::in);
	fin6.open("split_50/V_2.txt", ios::in);
	fin7.open("train_train.txt", ios::in);
	fin8.open("source.txt", ios::in);
	fin9.open("train_error.txt", ios::in);
	for (int i = 0; i < M1; i++){
		for (int j = 0; j < K; j++){
			fin1 >> U1.at(i, j);
		}
	...
	int a, b;
	float c;
	R1 = new vector< pair<int, float> > [M1];
	while(fin7 >> a >> b >> c){
		R1[a].push_back(make_pair(b, c));
	}

	R2 = new vector< pair<int, float> > [M2];
	while(fin8 >> a >> b >> c){
		R2[a].push_back(make_pair(b, c));

	}
	R_error = new vector< pair<int, float> > [M1];
	while(fin9 >> a >> b >> c){
		R_error[a].push_back(make_pair(b, c));
	...
	for (int i = 0; i < K; i++){
		...
		time1 = time(0);
		Guser = Matching(U1*sqrt(D1), U2*sqrt(D2), i, &T1);
		time2 = time(0);
		cout << "Matching 1 : spent " << time2 - time1 << " s" << endl;
		Gitem = Matching(V1*sqrt(D1), V2*sqrt(D2), i, &T2);
		time3 = time(0);
	...
	for (int l = 0; l < K; l++){
		...
		for(int i = 0; i < M1; i++){
			for(int j = 0; j < R_error[i].size(); j++){
				int user_temp = V_user(i);
				int item_temp = V_item(R_error[i][j].first);

				mat LA = U2.row(user_temp) * D2 * V2.row(item_temp).t();


				float value = R_error[i][j].second - LA(0, 0);
				LA.clear();
				error += value * value;
	...
	cerr << "push R_error" << endl;
	for(int i = 0; i < M1; i++){
		for(int j = 0; j < R_error[i].size(); j++){
			R1[i].push_back(make_pair(R_error[i][j].first, R_error[i][j].second));
		}
	}
	...
	//cout << V_user << endl;
	//cout << V_item << endl;
	vector<int> V2_user[M2];
	vector<int> V2_item[N2];
	...
	output.open("split_50/final_data.txt", ios::out);
	for(int i = 0; i < M1; i++){
		...
		for (int l = 0; l < Top; l++){
			...
			for(int i = 0; i < M1; i++){
				...
				for(int j = 0; j < R_error[i].size(); j++){
					...
					int item_temp = V2->at(R_error[i][j].first);
