...
public:
	...
	Vector3f(float32 var) : x(var), y(var), z(var) {}
	...
	inline float32 dot(const float32 var) const
	{
		return x * var + y * var + z * var;
	}

	inline float32 dot(const float32 varX, const float32 varY, const float32 varZ) const
	{
	...
	inline Vector3f& addLocal(const float32 varX, const float32 varY, const float32 varZ)
	{
		x += varX;
		y += varY;
		z += varZ;
	...
	inline boolean equals(const Vector3f &r) const
	{
		return x == r.x &&
			   y == r.y &&
			   z == r.z;
