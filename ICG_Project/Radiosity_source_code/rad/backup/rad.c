...
int             Debug = 0;
float           AreaLimit = 5.0;
float           SampleArea = 100.0;
float           ConvergeLimit = 50.0;
float           DeltaFFLimit = 0.025;
...
char            option[MAX_OPTION][3] =
{"-d", "-a", "-s", "-c", "-f"};

extern float    process_time();
...
ReadTriangle(FILE * fp)
{
	...
	while (1) {
		...
		/*
		 * // // Read twice for rad.tri format... // add by
		 * tshuang.... //
		 */
		...
		CalCenter(tp);
		CalArea(tp, v0, v1);	/* area */
		normalize(tp->n);	/* normal : must calculate after area
					 * !! */
...
PrintOut(char *fname, int loop)
{
	...
	for (i = 0; i < trinum; i++) {
		...
		**********************************************************/
		...
		fprintf(fout, "%f %f %f ",tp->p[0][0], tp->p[0][1], tp->p[0][2]);
		...
		fprintf(fout, "%f %f %f ", tp->p[2][0], tp->p[2][1], tp->p[2][2]);
...
ProcessOption(int argc, char *argv[])
{
	...
	if (argc < 3) {
		fprintf(stderr, "Usage: %s [-dascf] ifile ofile\n", argv[0]);
		fprintf(stderr, "       -d debuglevel    : default %i\n", Debug);
		fprintf(stderr, "       -a AreaLimit     : default %f\n", AreaLimit);
		fprintf(stderr, "       -s SampleArea    : default %f\n", SampleArea);
