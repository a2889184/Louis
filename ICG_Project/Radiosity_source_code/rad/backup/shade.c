...
/*
 * int AllocTriangle(void) { if (trinum == MaxTri) { fprintf(stderr, "\n***
 * Max Triangle exceed : %i ***\n", trinum); exit(-1); } return trinum++; }
 */
...
**************************************************/
float 
GetPointInTriangle(TrianglePtr tri, Vector p, float ff[3])
{
...
*******************************************************************/
void
PartitionSource(TrianglePtr s, TrianglePtr t1, TrianglePtr t2, int edge)
{
	Vector          p;
	int             i0, i1, i2;
...
double ExtendFF (Vector dir, TrianglePtr srctri, TrianglePtr destri, int count) {
	if (srctri->Brgb[0] == 255 || srctri->Brgb[1] == 255 || srctri->Brgb[2] == 255) {
		return 1;
	}
	if (count == 2) {
		return 0;
	...
	Vector src_normal;
	bool flag = false;
	float dot_value = 0, reflect_ff = 0, refract_ff = 0;
	CopyVector(dir, refract_dir);
	CopyVector(srctri->n, src_normal);
	normalize(src_normal);
	dot_value = InnerProd(dir, src_normal);
	Add1Vector(dir, -2 * dot_value, src_normal, reflect_dir);
	for (int i = 0; i < trinum; i++) {
		if (srctri->parent < 0) continue;
		if (RayHitted(srctri->c, reflect_dir, srctri->parent) != -1) {
			reflect_ff = ExtendFF(reflect_dir, &TriStore[i], srctri, count + 1);
			flag = true;
		}
		if (RayHitted(srctri->c, refract_dir, srctri->parent) != -1) {
			refract_ff = ExtendFF(dir, &TriStore[i], srctri, count + 1);
			flag = true;
		}
	}
	if (flag) {
		return 0.5 * refract_ff + 0.5 * reflect_ff + (srctri->Brgb[0] + srctri->Brgb[1] + srctri->Brgb[2]) / 765;
...
*******************************************************************/
double
CalFF(TrianglePtr srctri, int logsrc, TrianglePtr destri, int logdest, Vector p)
{
	double          ff = 0;
	...
	if (RayHitted(p, dir, logdest) == logsrc) {	/* if hit logical source */
		/*ctheta1 = CosTheta(dir, srctri->n);
		ctheta2 = -CosTheta(dir, destri->n);*/
		ff = ExtendFF(dir, srctri, destri, 0);
		printf("logsrc : %d, ff: %f\n", logsrc, ff);
		//ctheta1 * ctheta2 * srctri->area / (norm2(dir) * PI + srctri->area);
	}			/* if  ray hit. */
	if (ff < 0.0)
		return (0.0);
	return ff;
...
*******************************************************************/
double
AdaptCalFF(float ff, TrianglePtr srctri, int logsrc, TrianglePtr destri,
...
void
CalRadiosity(TrianglePtr srctri, TrianglePtr destri, float ff[3])
{
	int             v, c;
	float           lo, deltaB;

	for (v = 0; v < 3; v++) {
		for (c = 0; c < 3; c++) {	/* for color = R G B */
...
Shade(TrianglePtr srctri, int logsrc, TrianglePtr destri, int logdest, int realdest)
{
	...
	**********************************************************/
		...
		/* destri is a child triangle */
	{
		*destri = TriStore[t2ID];
		 ...
		 * correct.
		 */
		ff2[0] = AdaptCalFF(CalFF(srctri, logsrc, t2, logdest, t2->p[0]),
				    srctri, logsrc, t2, logdest, t2->p[0]);
	...
	**********************************************************/
	if ((neighborID = destri->neighbor[destedge]) >= 0) {
		...
		if (t4ID < realdest) {
			 ...
			 */
			for (v = 0; v < 3; v++)
				ff[v] = AdaptCalFF(CalFF(srctri, logsrc, t4, logdest, t4->p[v]),
				     srctri, logsrc, t4, logdest, t4->p[v]);
