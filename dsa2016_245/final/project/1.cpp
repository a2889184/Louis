...
void insert_string(vector<string> &dict, vector<long long> &dict_frequency, int start, int element_index, vector<int> &position,
	...
	if(x == 0){
		...
		long long size = dict.size();
		bool check = 0;
		long long r = 0;
		for(r = 0; r < size; r++){
			if(dict[r].compare(sentence) == 0) {
				check = 1;
				break;
			}
		}
		...
		if(check == 1){
			output++;
			node new_node(dict[r], dict_frequency[r]);
	...
	if(x < position.size()){
		for(int l = 0; l < possible_combine[x].size(); l++){
			if(possible_combine[x][l].size() == 0){
				insert_string(dict, dict_frequency, 0, element_index, position, possible_combine, 
					sentence + possible_combine[x][l], word, output, x + 1, continue_prepos_num, suitable);
			}
			else insert_string(dict, dict_frequency, 0, element_index, position, possible_combine, 
					sentence + possible_combine[x][l] + " ", word, output, x + 1, continue_prepos_num, suitable);
		}
	}
...
int main(){
	...
	vector<string> dict;
	vector<long long> dict_frequency;
...
*/
			...
			while(BigData >> inputElement){
				...
				if(element_index == index + 1) {
					...
					dict_frequency.push_back(stoll(inputElement, 0, 0));
					element_index = 0;
					dict_string = "";
				}
...
*/
	...
	while(cin.getline(inputLine, 100)){
		...
		if(havePre == 0){
			...
			insert_string(dict, dict_frequency, 0, element_index,position, possible_combine, "", word, output, 0 ,continue_prepos_num, 
				suitable);
			for(int i = 0; i < position.size(); i++){
